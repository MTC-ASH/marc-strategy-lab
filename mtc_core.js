// ═══════════════════════════════════════════════════════════════
// MTC Strategy Suite — Core Engine  (mtc_core.js)
// Requires: marc_data.js (defines window.RAW)
// Provides: backtest engine, stats, optimiser, charts, shared state
// Consumed by: mtc_lab.js (Research Lab) and mtc_bag.js (BAG Fund)
//
// FUNCTION INDEX:
//   filterByDates()        — filter results to date window
//   computeWeights()       — proximity + constraint engine
//   _doBacktest()          — full backtest run
//   calcStats()            — Sharpe/CAGR/DD/Sortino
//   runBacktest()          — UI-triggered backtest
//   runOptimise()          — parameter search
//   saveModel()            — save current params to model library
//   renderCumReturn()      — cumulative return chart
//   renderRiver()          — attribution river
//   renderHeatmap()        — monthly heatmap
//   renderDrawdown()       — drawdown chart
//   renderRolling()        — rolling 12m return
//   renderTurnover()       — churn per quarter
//   renderYearCards()      — year breakdown tab
//   openSnap()             — portfolio snapshot modal
//   renderAttributionTab() — attribution tab
//   renderMap()            — regime map
//   renderEligibility()    — eligibility timeline
//   renderSensitivity()    — sensitivity heatmap
//   updateRightPanel()     — right panel KPIs + verdict
//   refreshAll()           — re-render everything
//   initAssetConstraints() — build left panel sliders
//   syncDatePicker()       — date picker sync
// ═══════════════════════════════════════════════════════════════

// ── PROXY ─────────────────────────────────────────────────────────
const PROXY_URL = 'https://marc-agent.asher-8ca.workers.dev/';
const AGENT_KEY = ''; // stored securely in Cloudflare Worker env
...
// ── COLOUR CONSTANTS ──────────────────────────────────────────────
// Regime colours — updated to match v2 design system
const RC  = {
  Expansion:   '#00C97A',
  Deflation:   '#4D9FFF',
  Reflation:   '#FFB020',
...
const RBG = {
  Expansion:   'rgba(0,201,122,0.06)',
  Deflation:   'rgba(77,159,255,0.06)',
  Reflation:   'rgba(255,176,32,0.06)',
  Stagflation: 'rgba(255,77,109,0.06)'
};
// Asset type colours
const TC  = {
  Equity:    '#4D9FFF',
...
// ── PLOTLY THEME ──────────────────────────────────────────────────
// Updated to v2 design colours (#0D0F1A surface, #2E3148 subtle)
const PL = (extra = {}) => ({
  paper_bgcolor: 'rgba(0,0,0,0)',
  plot_bgcolor:  'rgba(0,0,0,0)',
  font:   {family: "'DM Mono',monospace", size: 9, color: '#6B6F8E'},
  margin: {l: 46, r: 10, t: 5, b: 28},
  xaxis:  {gridcolor: '#1C1F2E', linecolor: '#252840', tickfont: {size: 8}, zeroline: false},
  yaxis:  {gridcolor: '#1C1F2E', linecolor: '#252840', tickfont: {size: 8}, zeroline: false},
...
function regimeShapes(filtered) {
  ...
  for (const r of filtered) {
    if (r.quadrant !== cur) {
      if (cur && start) shapes.push({
        type: 'rect', xref: 'x', yref: 'paper',
        x0: start, x1: r.month, y0: 0, y1: 1,
        fillcolor: RBG[cur] || 'rgba(0,0,0,0)', line: {width: 0}
      });
  ...
  if (cur && start) shapes.push({
    type: 'rect', xref: 'x', yref: 'paper',
    x0: start, x1: filtered[filtered.length-1].month, y0: 0, y1: 1,
    fillcolor: RBG[cur] || 'rgba(0,0,0,0)', line: {width: 0}
  });
  return shapes;
}
...
function setFloor(asset, val) {
  assetFloors[asset] = +val/100;
  document.getElementById(`afv-${asset}`).textContent = val + '%';
}
function setCap(asset, val) {
  assetCaps[asset] = +val/100;
  document.getElementById(`acv-${asset}`).textContent = val + '%';
}
function updateSectorCap(type, val) {
  sectorCaps[type] = +val/100;
  document.getElementById(`scapv-${type}`).textContent = val + '%';
}
function liveParam(key, val) {
  if (lockedParams[key]) return;
  params[key] = parseFloat(val);
  document.getElementById(`dv-${key}`).textContent = key === 'n'
    ? Math.round(params.n)
...
function addComparison() {
  const s = document.getElementById('comp-select'), asset = s.value;
  if (!asset) return;
...
function renderCompList() {
  ...
  if (!comparisons.length) {
    el.innerHTML = `<div style="font-size:10px;color:var(--subtle);">None added</div>`;
    return;
  }
...
function computeWeights(rx, ry, availAssets, alpha, mcBeta, N) {
  ...
  for (const s of eligible) {
    if (s.floor > 0 && !top.find(h => h.asset === s.asset)) top.push(s);
  }
  const sum = top.reduce((s, x) => s + x.score, 0);
  for (const h of top) h.weight = sum > 0 ? h.score / sum : 1 / top.length;
  // Iterative projection — caps, floors, sector caps
  for (let iter = 0; iter < 20; iter++) {
    ...
    for (const [type, tot] of Object.entries(st)) {
      ...
      if (tot > cap + 1e-9) {
        const sc = cap / tot;
        for (const h of top) if (h.type === type) { h.weight *= sc; changed = true; }
      }
    }
    const total = top.reduce((s, h) => s + h.weight, 0);
    if (Math.abs(total - 1) > 1e-9) {
      const f = total > 0 ? 1/total : 1;
      for (const h of top) h.weight *= f;
      changed = true;
    }
    if (!changed) break;
  }
...
function _doBacktest(alpha, mcBeta, N) {
  ...
  for (let i = 0; i < ALL_RESULTS.length; i++) {
    ...
    for (const [a, wt] of Object.entries(w)) {
      const ret = ASSET_MONTHLY[a] ? (ASSET_MONTHLY[a][i] || 0) : 0;
      pr += wt * ret;
      ma[a] = wt * ret;
    }
    newMA.push(ma);
...
function calcStats(results) {
  ...
  for (const r of results) {
    if (r.port_cum > peak) peak = r.port_cum;
    const d = (peak - r.port_cum) / peak;
    if (d > mdd) mdd = d;
  }
  const calmar    = mdd > 0 ? cagr / mdd : 0;
  const rebalRows = results.filter(r => r.l1 > 0);
  const avgL1     = rebalRows.length > 0
    ? rebalRows.reduce((s, r) => s + r.l1, 0) / rebalRows.length : 0;
  return {totalRet, cagr, sharpe, sortino, calmar, mdd,
          best: Math.max(...rets), worst: Math.min(...rets), avgL1, n};
}

function objScore(stats, obj) {
  if (!stats || stats.cagr === undefined) return -Infinity;
  return {
    sharpe:   stats.sharpe,
    cagr:     stats.cagr,
    calmar:   stats.calmar,
    sortino:  stats.sortino,
    turnover: -stats.avgL1
  }[obj] ?? stats.sharpe;
}
...
function runOptimise() {
  ...
  setTimeout(() => {
    ...
    let best = {score: -Infinity, alpha: 1, mcbeta: 0, n: 15, sharpe: 0};
    let feasible = 0;
    ...
    sensiCache = {};
    _doBacktest(best.alpha, best.mcbeta, best.n);
...
// ═══════════════════════════════════════
// SAVE MODEL — new in v2
// Stores current params + stats to Supabase via worker
// Also stores to localStorage as fallback
// ═══════════════════════════════════════
async function saveModel() {
  ...
  const model = {
    id:        Date.now(),
    name:      `Model ${new Date().toLocaleDateString('en-AU', {day:'2-digit',month:'short',year:'2-digit'})} — α${params.alpha} N${Math.round(params.n)}`,
    saved_at:  new Date().toISOString(),
    params:    {alpha: params.alpha, mcbeta: params.mcbeta, n: Math.round(params.n)},
    date_range:{start: startDate, end: endDate},
    sector_caps:{...sectorCaps},
    stats: {
      sharpe:   +stats.sharpe.toFixed(3),
      cagr:     +stats.cagr.toFixed(4),
      mdd:      +stats.mdd.toFixed(4),
      sortino:  +stats.sortino.toFixed(3),
      calmar:   +stats.calmar.toFixed(3),
      totalRet: +stats.totalRet.toFixed(4),
      avgL1:    +stats.avgL1.toFixed(4),
      n_months: stats.n
    },
    // Store monthly returns for overlay rendering
    monthly_rets: filtered.map(r => ({month: r.month, ret: +r.port_ret.toFixed(6), cum: +r.port_cum.toFixed(6)}))
  };

  // Always save to localStorage first (instant, offline-safe)
  try {
    const existing = JSON.parse(localStorage.getItem('mtc_model_library') || '[]');
    // Cap at 10 models — remove oldest if needed
    if (existing.length >= 10) existing.shift();
    existing.push(model);
    localStorage.setItem('mtc_model_library', JSON.stringify(existing));
  } catch(e) {
    console.warn('localStorage save failed:', e);
  }

  // Also try Supabase via worker
  try {
    if (typeof _marcDB !== 'undefined' && _marcDB.jwt) {
      await fetch(_api() + '/db/models', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + _marcDB.jwt,
    ...
    // Supabase save is best-effort; localStorage is the source of truth
  }

  if (statusEl) {
    statusEl.textContent = `✓ Saved: ${model.name}`;
    statusEl.style.color = 'var(--green)';
    setTimeout(() => { if (statusEl) statusEl.textContent = ''; }, 4000);
  }

  // Notify BAG Fund model library if it's open
  if (typeof renderModelLibrary === 'function') renderModelLibrary();

  // Add alert
  if (typeof addAlert === 'function') {
    addAlert('info', 'Model saved', model.name);
  }
...
function loadModelLibrary() {
  try {
    return JSON.parse(localStorage.getItem('mtc_model_library') || '[]');
  } catch(e) {
    return [];
  }
}
...
function renderCumReturn() {
  const el = document.getElementById('chart-cumret');
  if (!el) return;
  const {filtered} = filterByDates(btResults, null);
  if (!filtered.length) return;
  const baseCum   = filtered[0].port_cum / (1 + filtered[0].port_ret);
  const startLabel = filtered[0].month.slice(0,4) + '-' +
    (parseInt(filtered[0].month.slice(5,7)) - 1).toString().padStart(2,'0');
  ...
  const traces  = [{
    x, y, name: 'Strategy', type: 'scatter', mode: 'lines',
    line: {color: '#E8E9F0', width: 2.5},
    hovertemplate: logScale ? '%{y:.2f}x<extra>Strategy</extra>' : '%{y:.1f}%<extra>Strategy</extra>'
  }];
  // Comparison benchmarks — rebased to window start
  for (const c of comparisons) {
    const si     = ALL_MONTHS.indexOf(filtered[0].month);
    const bBase  = si > 0
      ? (filtered[0][`comp_${c.asset}`] || 1) / (1 + (ASSET_MONTHLY[c.asset] ? ASSET_MONTHLY[c.asset][si] || 0 : 0))
      : 1;
    const cy = [logScale ? 1 : 0, ...filtered.map(r => {
      const idx = (r[`comp_${c.asset}`] || 1) / bBase;
      return logScale ? idx : (idx-1)*100;
    })];
    traces.push({
      x, y: cy, name: c.asset, type: 'scatter', mode: 'lines',
      line: {color: c.color, width: 1.5, dash: 'dot'},
      hovertemplate: logScale
        ? `%{y:.2f}x<extra>${c.asset}</extra>`
        : `%{y:.1f}%<extra>${c.asset}</extra>`
    });
  }
  const totalRet = (indexed[indexed.length-1] - 1) * 100;
  const metaEl   = document.getElementById('cum-meta');
  if (metaEl) metaEl.textContent =
    `${filtered[0].month} → ${filtered[filtered.length-1].month}  ·  ${totalRet>=0?'+':''}${totalRet.toFixed(1)}%${logScale?' · LOG':''}`;
  const yax = logScale
    ? {...PL().yaxis, type: 'log', tickformat: '.2f'}
    : {...PL().yaxis, ticksuffix: '%'};
  Plotly.react('chart-cumret', traces, {...PL({shapes}), yaxis: yax}, CFG);
  // Regime strip
  const strip = document.getElementById('regime-strip');
  if (strip) strip.innerHTML = filtered.map(r =>
    `<div style="flex:1;background:${RC[r.quadrant]||'#444'};opacity:0.7;" title="${r.month}: ${r.quadrant}"></div>`
  ).join('');
}
...
function renderRiver() {
  ...
  for (const asset of sortedAssets) {
    ...
    traces.push({
      x, y: filteredMa.map(ma => (ma[asset] || 0) * 100),
      name: asset, type: 'scatter', mode: 'none',
      fill: traces.length === 0 ? 'tozeroy' : 'tonexty',
      fillcolor: col + '44', line: {color: col, width: 0.5},
      hovertemplate: `${asset}: %{y:.2f}%<extra></extra>`,
      stackgroup: 'one'
    });
  }
  Plotly.react('chart-river', traces, {
    ...PL({margin: {l:46,r:10,t:4,b:28}}),
    yaxis: {...PL().yaxis, ticksuffix: '%'},
    showlegend: false
  }, CFG);
  // Click band → asset cap popup
  setTimeout(() => {
...
function renderHeatmap() {
  ...
  Plotly.react('chart-heatmap', [{
    z, x: MN, y: years, type: 'heatmap',
    colorscale: [[0,'#FF4D6D'],[0.3,'#3D0014'],[0.5,'#111320'],[0.7,'#001F12'],[1,'#00C97A']],
    zmid: 0, zmin: -20, zmax: 20,
    text, texttemplate: '%{text}', textfont: {size: 7, color: '#E8E9F0'},
    showscale: false,
    hovertemplate: '%{y} %{x}: %{z:.1f}%<extra></extra>'
  }], {
    ...PL({margin: {l:32,r:8,t:4,b:28}}),
    xaxis: {...PL().xaxis, side: 'bottom'},
...
function renderDrawdown() {
  ...
  for (const r of filtered) {
    if (r.port_cum > peak) peak = r.port_cum;
    dd.push(-((peak - r.port_cum) / peak) * 100);
  }
  const maxDD = Math.min(...dd);
  const ddMeta = document.getElementById('dd-meta');
  if (ddMeta) ddMeta.textContent = `Max DD: ${maxDD.toFixed(1)}%`;
  Plotly.react('chart-dd', [{
    x, y: dd, type: 'scatter', mode: 'lines', fill: 'tozeroy',
    line: {color: '#FF4D6D', width: 1},
...
function renderRolling() {
  ...
  const cols   = y.map(v => v >= 0 ? 'rgba(0,201,122,0.7)' : 'rgba(255,77,109,0.7)');
  Plotly.react('chart-rolling', [{
    x, y, type: 'bar',
    marker: {color: cols},
    hovertemplate: '%{x}: %{y:.1f}%<extra>12m return</extra>'
  }], {
    ...PL({margin: {l:46,r:10,t:4,b:28}, shapes}),
    yaxis: {...PL().yaxis, ticksuffix: '%'},
    showlegend: false
  }, CFG);
}
...
function renderTurnover() {
  const el = document.getElementById('chart-turnover');
  if (!el) return;
  const {filtered} = filterByDates(btResults, null);
  if (!filtered.length) return;
  const rebal = filtered.filter(r => r.l1 > 0);
  if (!rebal.length) return;
  const x   = rebal.map(r => r.quarter || r.month);
  const y   = rebal.map(r => r.l1 * 100);
  const avg = y.reduce((s, v) => s + v, 0) / y.length;
  Plotly.react('chart-turnover', [
    {
      x, y, type: 'bar',
      marker: {color: 'rgba(255,176,32,0.55)', line: {color: '#FFB020', width: 0.5}},
      hovertemplate: '%{x}: %{y:.1f}%<extra>Churn</extra>'
    },
    {
      x: [x[0], x[x.length-1]], y: [avg, avg],
      type: 'scatter', mode: 'lines',
      line: {color: '#6B6F8E', width: 1, dash: 'dot'},
      name: 'avg',
...
function renderYearCards() {
  const el = document.getElementById('year-cards');
  if (!el) return;
  const {filtered, filteredMa} = filterByDates(btResults, btResults._ma || MONTHLY_ATTR);
  const years = [...new Set(filtered.map(r => r.month.slice(0,4)))].sort();
  let html = '';
  for (const yr of years) {
    ...
    for (let i = 0; i < filtered.length; i++) {
      if (filtered[i].month.startsWith(yr)) yMas.push(filteredMa[i] || {});
    }
    ...
    const regC    = {};
    for (const r of yRows) regC[r.quadrant] = (regC[r.quadrant] || 0) + 1;
    const regBar  = Object.entries(regC).map(([reg, cnt]) =>
      `<div class="yr-rseg" style="width:${cnt/yRows.length*100}%;background:${RC[reg]};"></div>`
    ).join('');
    const totals  = {};
    for (const ma of yMas) for (const [a, v] of Object.entries(ma)) totals[a] = (totals[a]||0) + v;
    ...
    for (const qk of quarters) {
      ...
      const qMas  = [];
      for (let i = 0; i < filtered.length; i++) {
        if (filtered[i].quarter === qk && filtered[i].month.startsWith(yr)) qMas.push(filteredMa[i] || {});
      }
      const qBase = qMon[0].port_cum / (1 + qMon[0].port_ret);
      const qRet  = qMon[qMon.length-1].port_cum / qBase - 1;
      const qTotals = {};
      for (const ma of qMas) for (const [a, v] of Object.entries(ma)) qTotals[a] = (qTotals[a]||0) + v;
...
function toggleYear(yr) {
  document.getElementById(`ye-${yr}`).classList.toggle('open');
}
...
function openSnap(qk) {
  const q = quarterly[qk] || ALL_QUARTERLY[qk];
  if (!q) return;
  ...
  const qMas  = [];
  for (let i = 0; i < filtered.length; i++) {
    if (filtered[i].quarter === qk) qMas.push(filteredMa[i] || {});
  }
  let qRet = 0;
  if (qMons.length) {
    const base = qMons[0].port_cum / (1 + qMons[0].port_ret);
    qRet = qMons[qMons.length-1].port_cum / base - 1;
  }
  const qStats   = calcStats(qMons);
  const qContrib = {};
  for (const ma of qMas) for (const [a, v] of Object.entries(ma)) qContrib[a] = (qContrib[a]||0) + v;
...
// Asset cap popup from river/attr click
function showAssetCapPopup(assetName, event) {
  ...
  div.style.cssText = `position:fixed;z-index:300;left:${Math.min(x,window.innerWidth-220)}px;top:${Math.max(y,10)}px;background:var(--card);border:1px solid var(--border2);border-radius:var(--r-md);padding:12px;width:210px;box-shadow:0 8px 32px rgba(0,0,0,.5);`;
  const a = ASSETS.find(x => x.asset === assetName);
  div.innerHTML = `
    <div style="font-size:11px;font-weight:600;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;">
      ${assetName} <span class="badge badge-${a?.type}">${a?.type||''}</span>
      <button onclick="this.closest('.cap-popup').remove()" style="background:none;border:none;color:var(--subtle);cursor:pointer;font-size:15px;line-height:1;">×</button>
    </div>
    <div style="font-size:10px;color:var(--muted);margin-bottom:7px;">Max allocation cap</div>
    <div style="display:flex;gap:8px;align-items:center;">
      <input type="range" min="0" max="100" step="1" value="${cur}" style="flex:1;accent-color:var(--red);"
        oninput="setCap('${assetName}',this.value);document.getElementById('ac-${assetName}').value=this.value;document.getElementById('acv-${assetName}').textContent=this.value+'%';this.nextElementSibling.textContent=this.value+'%'"/>
...
function renderAttributionTab() {
  ...
  Plotly.react('chart-attr-bar', [{
    x: names, y: vals, type: 'bar',
    marker: {color: cols, opacity: 0.8},
    hovertemplate: '%{x}: %{y:.1f}%<extra></extra>'
  }], {
    ...PL({margin: {l:46,r:10,t:6,b:50}}),
    xaxis: {...PL().xaxis, tickangle: -45, tickfont: {size: 8}},
    yaxis: {...PL().yaxis, ticksuffix: '%'},
    showlegend: false
  }, CFG);
  setTimeout(() => {
  ...
  const atWrap = document.getElementById('attr-table-wrap');
  if (!atWrap) return;
  const rows = sorted.map(([asset, contrib]) => {
    const pctV  = (contrib*100).toFixed(2);
    const isPos = contrib >= 0;
    const bw    = maxAbs > 0 ? Math.abs(contrib)/maxAbs*80 : 0;
    const bar   = isPos
      ? `<span class="bpos" style="width:${bw}px;"></span>`
      : `<span class="bneg" style="width:${bw}px;"></span>`;
    const a     = ASSETS.find(x => x.asset === asset);
    return `<tr>
      <td style="font-weight:500;">${asset}
        <button onclick="showAssetCapPopup('${asset}',{clientX:200,clientY:200})"
          style="background:none;border:1px solid var(--border);border-radius:2px;color:var(--subtle);cursor:pointer;padding:1px 5px;font-size:9px;margin-left:3px;">cap</button>
      </td>
      <td><span class="badge badge-${a?.type}">${a?.type||'—'}</span></td>
      <td style="font-family:var(--mono);text-align:right;color:${isPos?'var(--green)':'var(--red)'};">${isPos?'+':''}${pctV}%</td>
      <td>${bar}</td>
    </tr>`;
  }).join('');
  atWrap.innerHTML = `<table class="attr-table"><thead><tr><th>Asset</th><th>Type</th><th>Total Contrib</th><th>Impact</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
function renderMap() {
  ...
  traces.push({
    x: trailX, y: trailY, mode: 'lines+markers', type: 'scatter',
    line: {color: '#252840', width: 1.5},
    marker: {color: trailC, size: 7, opacity: 0.9, line: {color: '#0B0D14', width: 1}},
    text: qKeys.map(k => {
      const q    = quarterly[k] || ALL_QUARTERLY[k];
      const top3 = (q.holdings || []).slice(0,3).map(h => `${h.asset} ${(h.weight*100).toFixed(0)}%`).join(', ');
      return `${k}<br>${q.quadrant}<br>${top3}`;
    }),
    hovertemplate: '%{text}<extra></extra>', name: 'Regime trail'
  });
  const grouped = ASSETS.reduce((g, a) => { (g[a.type] = g[a.type]||[]).push(a); return g; }, {});
  for (const [type, grp] of Object.entries(grouped)) {
    traces.push({
      x: grp.map(a => a.x), y: grp.map(a => a.y),
      mode: 'markers+text', type: 'scatter', name: type,
      marker: {color: TC[type], size: 9, opacity: 0.85, line: {color: '#0B0D14', width: 1}},
      text: grp.map(a => a.asset),
      textfont: {size: 8, color: TC[type]},
      textposition: 'top center',
      hovertemplate: '%{text}<extra>' + type + '</extra>'
    });
  }
  // Ambiguity zone
  const theta = Array.from({length: 65}, (_, i) => i/64*2*Math.PI);
  traces.push({
    x: theta.map(t => 1.5*Math.cos(t)), y: theta.map(t => 1.5*Math.sin(t)),
    mode: 'lines', type: 'scatter',
    line: {color: 'rgba(255,176,32,0.2)', width: 1.5, dash: 'dot'},
    name: 'Ambiguous zone', hoverinfo: 'skip', showlegend: true
  });
  const shapes = [
    {type:'line',xref:'x',yref:'paper',x0:0,x1:0,y0:0,y1:1,line:{color:'#1C1F2E',width:1}},
    {type:'line',xref:'paper',yref:'y',x0:0,x1:1,y0:0,y1:0,line:{color:'#1C1F2E',width:1}},
  ];
  const annotations = [
    {x:3.5,y:4.8,text:'REFLATION',showarrow:false,font:{size:9,color:RC.Reflation,family:'DM Mono'},opacity:.3},
    {x:3.5,y:-4.8,text:'EXPANSION',showarrow:false,font:{size:9,color:RC.Expansion,family:'DM Mono'},opacity:.3},
    {x:-3.5,y:4.8,text:'STAGFLATION',showarrow:false,font:{size:9,color:RC.Stagflation,family:'DM Mono'},opacity:.3},
    {x:-3.5,y:-4.8,text:'DEFLATION',showarrow:false,font:{size:9,color:RC.Deflation,family:'DM Mono'},opacity:.3},
  ];
  Plotly.react('chart-map', traces, {
    paper_bgcolor: '#0D0F1A', plot_bgcolor: '#0D0F1A',
    font: {family: "'DM Mono',monospace", size: 9, color: '#6B6F8E'},
    margin: {l:40,r:10,t:10,b:30},
    xaxis: {gridcolor:'#1C1F2E',linecolor:'#252840',range:[-6.5,6.5],title:{text:'← Growth →',font:{size:9}}},
    yaxis: {gridcolor:'#1C1F2E',linecolor:'#252840',range:[-5.5,5.5],title:{text:'← Inflation →',font:{size:9}},scaleanchor:'x',scaleratio:1},
    shapes, annotations,
...
function toggleEligibility() {
  ...
  if (el)  el.style.display = showElig ? 'block' : 'none';
  if (btn) {
...
function renderEligibility() {
  ...
  const x = qKeys;
  const y = qKeys.map(qk => (quarterly[qk] || ALL_QUARTERLY[qk])?.avail_count || 0);
  const cols = qKeys.map(qk => TC[{
    Expansion:'ETF', Deflation:'Commodity', Reflation:'Equity', Stagflation:'Crypto'
  }[(quarterly[qk] || ALL_QUARTERLY[qk])?.quadrant]] || '#888');
  Plotly.react('chart-eligibility', [{
    x, y, type: 'bar',
    marker: {color: cols, opacity: 0.7},
    hovertemplate: '%{x}: %{y} assets<extra></extra>'
  }], {
    ...PL({margin: {l:40,r:10,t:10,b:36}}),
    yaxis: {...PL().yaxis, title: {text:'Eligible',font:{size:8}}},
    showlegend: false,
    title: {text:'Asset Eligibility Over Time',font:{size:10,color:'#6B6F8E'}}
  }, CFG);
}
...
function renderSensitivity() {
  ...
  if (xAxis === yAxis) {
    yAxis = xAxis === 'alpha' ? 'n' : 'alpha';
    document.getElementById('sens-y').value = yAxis;
  }
  const vals = {
    alpha:  [0.2,0.5,0.8,1.0,1.5,2.0,3.0,4.0],
    n:      [5,8,10,12,15,18,20],
    mcbeta: [0,0.25,0.5,0.75,1.0]
  };
  ...
  for (const yv of yVals) {
    ...
    for (const xv of xVals) {
      ...
      const p = {alpha: params.alpha, n: params.n, mcbeta: params.mcbeta};
      p[xAxis] = xv; p[yAxis] = yv; p[fixK] = fixV;
  ...
  _doBacktest(params.alpha, params.mcbeta, params.n);
  const annotations = [{
    x: params[xAxis], y: params[yAxis], text: '✕',
    showarrow: false, font: {size: 14, color: '#E8E9F0'}
  }];
  ...
  Plotly.react('chart-sensitivity', [{
    ...
    colorscale: [[0,'#FF4D6D'],[0.3,'#3D0014'],[0.5,'#111320'],[0.7,'#001F12'],[1,'#00C97A']],
    colorbar: {tickfont: {size:8,color:'#6B6F8E'}, thickness: 10, len: 0.8},
    hovertemplate: `${xAxis}=%{x}, ${yAxis}=%{y}: %{z:.2f}<extra></extra>`
  }], {
    paper_bgcolor: '#0D0F1A', plot_bgcolor: '#0D0F1A',
    font: {family: "'DM Mono',monospace", size: 9, color: '#6B6F8E'},
    margin: {l:50,r:80,t:30,b:50},
    xaxis: {title: {text:xAxis,font:{size:9}}, tickfont:{size:9}},
    yaxis: {title: {text:yAxis,font:{size:9}}, tickfont:{size:9}},
    annotations,
    title: {text:`${lbl[metric]} sensitivity · fixed: ${fixK}=${fixV}`,font:{size:10,color:'#6B6F8E'}}
  }, CFG);
}
...
function updateRightPanel() {
  ...
  set('sk-ret',   pct(stats.totalRet, 1));
  set('sk-cagr',  pct(stats.cagr, 1));
  set('sk-sharpe', fx(stats.sharpe));
  set('sk-mdd',   '-' + xpct(stats.mdd, 1));
  set('sk-best',  '+' + xpct(stats.best, 1));
  set('sk-worst', xpct(stats.worst, 1));
  const isFullHistory = startDate === ALL_MONTHS[0] && endDate === ALL_MONTHS[ALL_MONTHS.length-1];
  set('sel-lbl', isFullHistory ? 'Full history' : `${startDate} → ${endDate}`);

  // Topbar KPIs (Lab mode)
  const multiple = filtered[filtered.length-1].port_cum / (filtered[0].port_cum / (1 + filtered[0].port_ret));
  set('ts-total',   multiple.toFixed(1) + 'x');
  set('ts-cagr',    pct(stats.cagr, 1));
  set('ts-sharpe',  fx(stats.sharpe));
  set('ts-mdd',     '-' + xpct(stats.mdd, 1));
  set('ts-sortino', fx(stats.sortino));
  set('ts-churn',   xpct(stats.avgL1, 0));

  const lastR  = filtered[filtered.length-1];
  const rp     = document.getElementById('ts-regime');
  if (rp) { rp.textContent = lastR.quadrant; rp.className = `rpill rpill-${lastR.quadrant.charAt(0)}`; }

  // Regime mix
  const regC = {};
  for (const r of filtered) regC[r.quadrant] = (regC[r.quadrant]||0) + 1;
  const rmb = document.getElementById('regime-mix-bar');
  const rml = document.getElementById('regime-mix-leg');
  if (rmb) rmb.innerHTML = Object.entries(regC).map(([reg, cnt]) =>
    `<div style="flex:${cnt};background:${RC[reg]};"></div>`
  ).join('');
  if (rml) rml.innerHTML = Object.entries(regC).map(([reg, cnt]) =>
    `<span style="color:${RC[reg]}">${reg.slice(0,3)} ${(cnt/filtered.length*100).toFixed(0)}%</span>`
  ).join('');
  ...
  for (const qk of qKeys) {
    const q = quarterly[qk] || ALL_QUARTERLY[qk];
    if (!q || !q.holdings) continue;
    const qMons = filtered.filter(r => r.quarter === qk);
    const qMas  = [];
    for (let i = 0; i < filtered.length; i++) {
      if (filtered[i].quarter === qk) qMas.push(filteredMa[i] || {});
    }
    const hSorted = [...q.holdings].sort((a, b) => a.dist - b.dist);
    ...
    vx.push(parseFloat(hSorted[0]?.dist?.toFixed(2)));
    vy.push(parseFloat(qTotalRet.toFixed(2)));
    vt.push(`${qk} ${q.quadrant}`);
  }
  const vpct = total > 0 ? Math.round(wins/total*100) : 0;
  const vs   = document.getElementById('verdict-score');
  if (vs) {
    vs.textContent = vpct + '%';
    vs.style.color = vpct >= 60 ? 'var(--green)' : vpct >= 45 ? 'var(--amber)' : 'var(--red)';
  }
  set('verdict-lbl', `of ${total} quarters: closest assets beat distant`);

  const vEl = document.getElementById('chart-verdict');
  if (vEl) Plotly.react('chart-verdict', [{
    x: vx, y: vy, mode: 'markers', type: 'scatter',
    marker: {color: vy.map(v => v>=0?'#00C97A':'#FF4D6D'), size: 5, opacity: 0.7},
    text: vt,
    hovertemplate: '%{text}<br>Dist: %{x}, Ret: %{y:.1f}%<extra></extra>'
  }], {
    paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)',
    font: {family:"'DM Mono',monospace",size:8,color:'#6B6F8E'},
    margin: {l:40,r:8,t:4,b:28},
    xaxis: {gridcolor:'#1C1F2E',linecolor:'#252840',tickfont:{size:8},title:{text:'Min dist to regime',font:{size:8}}},
    yaxis: {gridcolor:'#1C1F2E',linecolor:'#252840',tickfont:{size:8},ticksuffix:'%'},
  ...
  const mkRow  = ([asset, contrib]) => {
    ...
    return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;">
      ...
      <span style="font-family:var(--mono);font-size:11px;min-width:48px;text-align:right;color:${contrib>=0?'var(--green)':'var(--red)'}">
        ${contrib>=0?'+':''}${(contrib*100).toFixed(1)}%
      </span>
    </div>`;
  };
  const tcEl = document.getElementById('top-contrib');
  const tdEl = document.getElementById('top-detract');
  if (tcEl) tcEl.innerHTML = sorted.filter(([,v]) => v > 0).slice(0,6).map(mkRow).join('');
  if (tdEl) tdEl.innerHTML = sorted.filter(([,v]) => v < 0).slice(0,4).map(mkRow).join('');
  ...
  set('elig-count', avgE.toFixed(0) + ' / ' + ASSETS.length);
  const ef = document.getElementById('elig-fill');
  if (ef) ef.style.width = (avgE/ASSETS.length*100) + '%';
}
...
function showTab(id) {
  document.querySelectorAll('.tab-btn').forEach((t, i) => {
    t.classList.toggle('active', ['main','years','attribution','map','sensitivity'][i] === id);
  });
...
function toggleGuide() {
  ...
  if (gb) gb.style.display = guideOpen ? 'grid' : 'none';
  if (gt) gt.textContent   = guideOpen ? 'hide' : 'show';
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const years  = [];
  for (let y = 2010; y <= 2027; y++) years.push(y);
  ...
  function buildSelects(prefix, defaultVal) {
    ...
    mSel.innerHTML = MONTHS.map((m, i) =>
      `<option value="${String(i+1).padStart(2,'0')}" ${i+1===dMonth?'selected':''}>${m}</option>`
    ).join('');
    ySel.innerHTML = years.map(y =>
      `<option value="${y}" ${y===dYear?'selected':''}>${y}</option>`
    ).join('');
  }
